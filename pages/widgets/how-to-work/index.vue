  ...
  <div>
    ...
    <div v-else class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title m-0">How It Work List</h5>

        <nuxt-link :to="{ name: 'widgets-how-to-work-create' }" class="btn btn-info btn-sm">
        ...
        <DataTable id="dataTable" :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy" class="">
          <tbody>
            <tr v-for="(value, i) in values" :key="i">
              ...
              <td>
                <nuxt-link :to="{ name: 'widgets-how-to-work-id-edit', params: { id: value.id } }" rel="tooltip"
...
export default {
  ...
  methods: {
    getData(url = 'widgets/how-it-works/get-list') {
      this.tableData.draw++;
    ...
    async statusChange(params) {
      await this.$axios.$post('widgets/how-it-works/change-status/' + params.id, params)
        .then(response => {
          this.$izitoast.success({
            title: 'Success !!',
            message: 'How it work status change successfully!'
          });
          this.getData()
        }).catch(error => {
          alert(error);
    ...
    async deleteItem(id) {
      ...
      if (result) {
        await this.$axios.$post('widgets/how-it-works/delete/' + id)
          .then(response => {
            this.getData();
            this.$izitoast.success({
              title: 'Success !!',

...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async deActivate(id) {
        await this.$axios.$post('/settings/utility/category/deactive/' + id, this.form,)
          .then(response => {
      ...
      async active(id) {
        await this.$axios.$post('/settings/utility/category/active/' + id, this.form,)
          .then(response => {
            this.$toast.success('Utility category activated successfully!');
            this.$router.push({name: 'settings-utilities-categories'});
          })
          .catch(error => {
            if (error.response.status == 422) {

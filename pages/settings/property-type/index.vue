  ...
  <div>
    ...
    <div v-else class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <nuxt-link :to="{ name: 'settings-property-type-create' }" class="btn btn-sm btn-info">
...
export default {
  ...
  methods: {
    getData(url = '/settings/property-type/list') {
      this.tableData.draw++;
    ...
    async deleteItem(id) {
      ...
      if (result) {
        await this.$axios.$post('settings/property-type/delete/' + id)
          .then(response => {
            this.getData();
    ...
    async statusChange(params) {
      await this.$axios.$post('/settings/property-type/change-status/' + params.id, params)
        .then(response => {
          this.$izitoast.success({
            title: 'Success !!',

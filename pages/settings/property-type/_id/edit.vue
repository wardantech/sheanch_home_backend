...
<script>
  export default {
    ...
    async created() {
      await this.$axios.$get('/settings/property-type/edit/' + this.$route.params.id)
        .then(response => {
    ...
    methods: {
      async update() {
        this.isDisable = true;
        await this.$axios.$post('/settings/property-type/update/' + this.$route.params.id, this.form,)
          .then(response => {

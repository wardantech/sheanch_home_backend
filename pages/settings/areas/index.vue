  ...
  <div>
    ...
    <div v-else class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="card-title m-0">Area lists</h5>

        <nuxt-link :to="{ name: 'settings-areas-create' }" class="btn btn-sm btn-info">
        ...
        <DataTable id="dataTable" :columns="columns" :sortKey="sortKey" :sortOrders="sortOrders" @sort="sortBy" class="">
          <tbody>
            <tr v-for="(value, i) in values" :key="value.id">
              ...
              <td>{{ value.districts.name }}</td>
              <td>{{ value.thanas.name }}</td>
              <td>
                <nuxt-link :to="{ name: 'settings-areas-id-edit', params: { id: value.id } }" rel="tooltip"
...
export default {
  ...
  methods: {
    ...
    async deleteItem(id) {
      ...
      if (result) {
        await this.$axios.$delete('areas/delete/' + id)
          .then(response => {
            this.getData();
